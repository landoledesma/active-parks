...
| `.gitignore` | config | Excludes `.venv/`, `.playwright-cli/`, `.claude/`, caches. |
...
| `sessions/` | record | Claude Code session transcripts — the raw work record of how the pipeline was built. |
...
### `rfp-scraper/out/` — artifacts (committed as produced; regenerated by running the pipeline)
